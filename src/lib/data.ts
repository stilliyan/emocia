...
export type Product = { id:string; name:string; slug:string; short_description:string|null; description:string|null; category_id:string; status:"draft"|"published"|"archived"; featured:boolean; product_code:string|null; sizes:string[]; color:string|null; material:string|null; collection:string|null; year:number|null; price:number|null; silhouette:"a-line"|"mermaid"|"princess"|"straight"|null; accessory_category:"veils"|"hair"|"jewellery"|"gloves"|"glasses"|"shoes"|"decorations"|null; seo_title:string|null; meta_description:string|null; updated_at:string; categories?:{name:string}|null; product_images?:ProductImage[] };
...
export async function getSiteSettings():Promise<SiteSettings>{const fallback:SiteSettings={shop_name:"Емоция",default_seo_title:null,default_meta_description:null,contact_phone:null,contact_email:null,address:null,working_hours:null,instagram_url:null,facebook_url:null,tiktok_url:null,maps_url:null};if(!hasSupabaseConfig())return fallback;const{supabase}=await requireAdmin();const result=await supabase.from("site_settings").select("shop_name,default_seo_title,default_meta_description,contact_phone,contact_email,address,working_hours,instagram_url,facebook_url,tiktok_url,maps_url").eq("id",true).maybeSingle();if(result.error?.code==="42703"||result.error?.code==="PGRST204"){const legacy=await supabase.from("site_settings").select("shop_name,default_seo_title,default_meta_description,contact_phone,contact_email,address,working_hours,instagram_url,facebook_url,maps_url").eq("id",true).maybeSingle();if(legacy.error)throw new Error(`Настройките не могат да бъдат заредени: ${legacy.error.message}`);return{...fallback,...legacy.data};}if(result.error)throw new Error(`Настройките не могат да бъдат заредени: ${result.error.message}`);return result.data??fallback;}
...
export async function getAppointmentManagementData():Promise<{appointments:Appointment[];blocks:AppointmentBlock[];settings:AppointmentSettings}>{
  ...
  const firstError=appointmentsResult.error??blocksResult.error??settingsResult.error;
  if(firstError){
    if(firstError.code==="42P01"||firstError.code==="PGRST205"||firstError.code==="42703"||firstError.code==="PGRST204")throw new Error("Записванията изискват Supabase migration 202607230002.");
    throw new Error(`Записванията не могат да бъдат заредени: ${firstError.message}`);
  }
  const fallback:AppointmentSettings={timezone:"Europe/Sofia",duration_minutes:60,buffer_minutes:15,maximum_companions:3,booking_window_days:120,minimum_notice_hours:24,weekly_schedule:{"1":[["10:00","19:00"]],"2":[["10:00","19:00"]],"3":[["10:00","19:00"]],"4":[["10:00","19:00"]],"5":[["10:00","19:00"]],"6":[["10:00","19:00"]],"7":[]}};
  return{appointments:(appointmentsResult.data??[])as Appointment[],blocks:(blocksResult.data??[])as AppointmentBlock[],settings:(settingsResult.data??fallback)as AppointmentSettings};
}
